...
$$w_I^{(k)} = 1 + \lambda \cdot \mathcal{D}_{\text{Topo}}(C_I^{(k)}, \widehat{C}_I^{(k)})$$
...
$$\mathcal{L}_{\text{seg}}(I^{(k)}) = \text{DL}(Y_I^{(k)}, \widehat{Y}_I^{(k)}) + \sum_{q \in \text{HARD}_I^{(k)}} \text{BCE}(q, \widehat{q})$$
...
```
@article{caki2026tghem,
  title   = {Topology-guided hard example mining for cell detection},
  author  = {Çakı, Onur and Unver, Sinan and Dur Karasayar, Ayse Humeyra and Aydin Mericoz, Cisel and Bulutay, Pinar and Kapucuoglu, Nilgun and Eren, Handan and Dilbaz, Omer Faruk and Osmanli, Javidan and Yetkili, Burhan Soner and Kulac, Ibrahim and Gunduz-Demir, Cigdem},
